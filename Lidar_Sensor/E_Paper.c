...
const int wake_up = PE4;
const int reset = PE5;
const int epd_UART_TX = PB1;
const int epd_UART_RX = PB0;
...
void delayMicroseconds(uint32_t value){
    SysCtlDelay(value*(SysCtlClockGet()/3000000));
}
void delay(uint32_t value){
    SysCtlDelay(value*(SysCtlClockGet()/3000));
}
...
void epd_init(void)
{
    GPIOSet(wake_up,OUTPUT);
    GPIOSet(reset,OUTPUT);
    GPIOSet(epd_UART_TX,UART);
    GPIOSet(epd_UART_RX,UART);
    UARTSetBaud(epd_UART_TX,UART_Baudrate);
    //UARTEnableInt(epd_UART_RX);
}
/*******************************************************************************
...
{
    GPIOWrite(wake_up, LOW);
    delayMicroseconds(10);
    GPIOWrite(wake_up, HIGH);
    delayMicroseconds(500);
    GPIOWrite(wake_up, LOW);
    delay(10);
}
