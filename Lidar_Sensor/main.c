...
// =============================== User Definition =================================
#define             LeftMotor_Dir              PC6              //output
#define             LeftMotor_PWM              PC5              //pwm
#define             RightMotor_Dir             PB3              //output
#define             RightMotor_PWM             PC4              //pwm
#define             Front_ADC                  PE1              //adc
#define             Right_ADC                  PD3              //adc pd1 is ont working, need to check the pcb
...
#define             Reflectro_Sensor           PD2              //input
//-----LCD-----
...
int main(void){
    ...
    GPIOInit();
    LCDInit();
    UARTStringPut(User_UART_TX,"===== Tiva-c Initiated =====");
    UARTStringPut(Bluetooth_UART_TX,"===== Tiva-c Initiated =====");

    LCD_LED_ON();
    LCDClear();
    //LCDWriteString("I Love Chicken");
    LCDWriteFace();
    BIOS_start();
    return 0;
}
...
void DefaultBluetoothSetup(void){
    ...
    UARTStringPut(Bluetooth_UART_TX,"Reset");
}
void Bluetooth_UART_Hwi(void){
    ...
    if(UART_Contrler){
        ...
        case '#': UART_Contrler = true; UARTStringPut(Bluetooth_UART_TX,"Controller Mode"); break;
        }
    }
}
...
void Contrler_Task(void){
    ...
    while(1){
        ...
        switch(cmd){
            case '1': MoveForward(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Up"); break;
            case '2': MoveBackward(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Down"); break;
            case '3': MoveRight(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Right"); break;
            case '4': MoveLeft(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Left"); break;
            case '5': MoveSlightRight(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Up Right");break;
            case '6': MoveSlighLeft(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Up Left"); break;
            case '7': MoveBackRight(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Down Right"); break;
            case '8': MoveBackLeft(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Down Left"); break;
            case 'R': RotateRight(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Rotate Right"); break;
            case 'L': RotateLeft(); StartMoving(); UARTStringPut(Bluetooth_UART_TX,"Rotate Left"); break;
            case 'S': SetSpeed(UARTIntegerGet(Bluetooth_UART_RX)); UARTStringPut(Bluetooth_UART_TX,"Set Speed to: "); UARTIntegerPut(Bluetooth_UART_TX,Right_Speed); break;
            case '!': DefaultBluetoothSetup(); break;
        }
